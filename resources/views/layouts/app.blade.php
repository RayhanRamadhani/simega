    ...
    <div class="flex h-screen overflow-hidden">
        <div id="sidebar" class="sidebar fixed top-0 left-0 w-48 h-full bg-white shadow-md transform -translate-x-full transition-transform duration-300 ease-in-out">
            <div class="p-5 flex items-center">
                ...
                <span class="font-bold text-xl">SIMEGA</span>
            </div>
            <nav class="mt-5 text-base font-semibold">
                <ul>
                    ...
                    <a href="{{ route('chatbot') }}" 
                        class="px-6 py-1 rounded-full shadow-md bg-white border border-blue-500 text-transparent bg-clip-text inline-flex items-center w-fit"
                        style="background-image: linear-gradient(to right, #FF00BF, #067AFF);">
                        
                        <svg xmlns="http://www.w3.org/2000/svg" 
                            fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="size-6 mr-2">
                        <defs>
                            <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
                            <stop offset="0%" stop-color="#FF00BF"/>
                            <stop offset="100%" stop-color="#067AFF"/>
                            </linearGradient>
                        </defs>
        ...
        <div class="flex-1 flex flex-col">
            <header class="bg-white p-2 fixed top-0 left-0 w-full z-10">
                <div class="flex items-center justify-between">
                    ...
                    @if (Auth::check() && isset($isuserexpired) && isset($isexxpired))
                        @if ($isuserexpired > $isexxpired)
                        @endif
                    @endif

                    <div class="flex items-center gap-3">
                        @if($user = Auth::user()->tier === 'PRO')
                            <button class="border border-blue-500 text-blue-500 px-4 py-1 rounded-lg">{{ Auth::user()->tier }}</button>
                        @else
                            <a href="{{ route("payment.process") }}">
                                <button class="border border-gray-500 text-gray-500 px-4 py-1 rounded-lg">{{ Auth::user()->tier }}</button>
                            </a>
                        @endif
