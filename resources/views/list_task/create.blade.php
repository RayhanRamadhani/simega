...
<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    <form action="{{ route('list-task.store') }}" method="POST" class="bg-white rounded-2xl shadow-lg w-[90%] max-w-md p-6 relative">
        ...
        <label class="block mb-4">
            ...
            <input type="date" name="date" class="mt-1 w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
        </label>
        <label class="block mb-4">
            <span class="text-sm font-semibold">Waktu</span>
            <input type="time" name="time" class="mt-1 w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
        </label>
